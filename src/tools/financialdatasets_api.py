...
def get_prices(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    if cached_data := _cache.get_prices(ticker):
        ...
        if filtered_data:
            return prices_to_df(filtered_data)
...
def get_financial_metrics(
    ...
    if cached_data := _cache.get_financial_metrics(ticker):
        ...
        if filtered_data:
            return pd.DataFrame([m.model_dump() for m in filtered_data[:limit]])
    ...
    return pd.DataFrame([m.model_dump() for m in financial_metrics])
...
def get_market_cap(ticker: str, end_date: str) -> float | None:
    ...
    if metrics.empty:
        return None
    return metrics.iloc[0].get('market_cap')
...
def get_price_data(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    """Get price data using the prices function."""
